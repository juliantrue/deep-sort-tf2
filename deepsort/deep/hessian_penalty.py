...
def hessian_penalty(
    ...
    **G_kwargs
    ...
    for _ in range(k):  # Iterate over each (N, z.shape) tensor in xs
        ...
        )  # Appends a tensor with shape equal to G(z).shape
...
def rademacher(shape):
    ...
    tfp_major_version, tfp_minor_version = tfp.__version__.split(".")[
        :2
    ]  # Handle annoying TF deprecation...
...
def _test_hessian_penalty():
    ...
    print(
        "Approximation: %s" % predicted_np
    )  # This should be close to ground_truth, but not exactly correct
    print(
        "Difference: %s"
        % [
            str(100 * abs(p - gt) / gt) + "%"
            for p, gt in zip(predicted_np, ground_truth_np)
        ]
    )
